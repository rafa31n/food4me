...
$query = $cn->prepare("SELECT * FROM reservaciones r INNER JOIN mesas m ON m.id_mesa = r.mesa_reservada  
INNER JOIN zonas z ON z.id_zona = m.zona WHERE r.estado = 1 AND r.usuario = ?");
$query->execute(array($_SESSION['id_usuario']));

$query2 = $cn->prepare("SELECT * FROM reservaciones r INNER JOIN mesas m ON m.id_mesa = r.mesa_reservada  
INNER JOIN zonas z ON z.id_zona = m.zona WHERE r.estado = 2 AND r.usuario = ?");
